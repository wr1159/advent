...
export async function queryAttendees(userId: string, eventId: string): Promise<any[] | undefined> {
  try {
    const attendeesRef = collection(firestore, `users/${userId}/events/${eventId}/attendees`);
