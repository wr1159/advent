...
export default async function deleteEvent(user_id: string, event_id: string) {
  ...
  await getAllTemplateIds(user_id, event_id).then((Ids) =>
    Ids.forEach((Id) =>
      deleteDoc(
        doc(firestore, `users/${user_id}/events/${event_id}/templates/${Id}`)
      )
    )
  );
  ...
  await attendeesSnapshot.forEach((attendee) =>
    deleteDoc(
      doc(
        firestore,
  ...
  // Delete the doc
  await deleteDoc(doc(firestore, `users/${user_id}/events/${event_id}`));
}
